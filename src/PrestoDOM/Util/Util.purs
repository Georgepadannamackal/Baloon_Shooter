...
render dom state = do
  ...
  pure { updateState : (updateState push), stateBeh, push : push }
...
getBeh :: forall a. Dynamic a -> Behavior a
getBeh (Dynamic { beh }) = beh

getEvent :: forall a. Dynamic a -> Event a
getEvent (Dynamic { event }) = event
